...
export class AdvanceComponent implements OnInit {
  ...
  async fetchSampleDynamic() {
    /*
    const req = new XMLHttpRequest();
    req.open("GET", `http://localhost:9000/getVideos`);
    ...
    req.send();*/
    const requ = await fetch("http://localhost:9000/getVideos");
    console.log(requ);
    const dat = await requ.json();
    console.log(dat);
    this.dynamicRows = dat;
  }
